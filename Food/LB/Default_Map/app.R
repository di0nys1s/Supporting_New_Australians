    ...
    if (!is.null(selected_suburb)){
      ...
      if (!typeof(data) == 'logical'){
        ...
        if (length(data[[1]] >0)){
          ...
          existing_business = HTML({paste('<p style="color:#3A479B;  font-size: 15px;">',paste('Number of Existing Business: <strong>',no),'</strong></p>')})
          ...
          existing_business = HTML('<p style="color:#3A479B; font-size: 15px;">Number of Existing Business: <strong>0</strong></p>')
          output$existing_business <- renderText({ 
            existing_business
          })
          output$restaurant_text <- renderText({
